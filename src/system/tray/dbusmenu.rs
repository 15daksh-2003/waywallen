...
impl DBusMenu {
    pub fn new(app: Arc<DaemonContext>) -> Self {
        Self { app }
    }
}
...
impl DBusMenu {
    ...
    async fn event(
        ...
        match id {
            ...
            ID_PAUSE => {
                if let Err(e) = application::toggle_pause_all(&app).await {
                    log::warn!("tray toggle pause: {e}");
                }
            }
            ID_MUTE => {
                if let Err(e) = application::toggle_mute_all(&app).await {
                    log::warn!("tray toggle mute: {e}");
                }
            }
            ID_RESCAN => {
                if let Err(e) = application::rescan(&app).await {
                    log::warn!("tray rescan: {e}");
                }
            }
            ID_SHUFFLE => {
                let was_on = matches!(app.queue.lock().await.mode, crate::playback::Mode::Shuffle);
                application::set_shuffle(&app, !was_on).await;
            }
            ID_ROT_OFF | ID_ROT_30S | ID_ROT_1M | ID_ROT_5M | ID_ROT_15M | ID_ROT_1H => {
                if let Some((_, _, secs)) = rotate_options()
                    ...
                    .find(|(rid, _, _)| *rid == id)
                {
                    application::set_rotation_interval(&app, secs).await;
                }
            }
            ID_QUIT => {
                app.shutdown_now();
...
async fn snapshot_menu_state(app: &Arc<DaemonContext>) -> MenuState {
...
/// Best-effort `LayoutUpdated` emission. Called from `application::*`
/// helpers when state that affects checkmark / radio rendering changes
pub async fn notify_menu_changed(app: &Arc<DaemonContext>) {
    ...
    let iface = match conn
        .object_server()
        .interface::<_, DBusMenu>(super::MENU_PATH)
        .await
    {
        Ok(i) => i,
        Err(_) => return,
...
async fn dispatch_click(app: &Arc<DaemonContext>, id: i32) -> zbus::fdo::Result<()> {
    match id {
        ...
        ID_MUTE => {
            let _ = application::toggle_mute_all(app).await;
        }
        ID_RESCAN => {
            let _ = application::rescan(app).await;
        }
        ID_SHUFFLE => {
            let was_on = matches!(app.queue.lock().await.mode, crate::playback::Mode::Shuffle);
            application::set_shuffle(app, !was_on).await;
