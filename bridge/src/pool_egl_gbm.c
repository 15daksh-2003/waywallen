...
typedef void (*ww_pfn_glTexParameteri)(unsigned int target, unsigned int pname, int param);
...
typedef void (*ww_pfn_glFramebufferTexture2D)(unsigned int target, unsigned int attachment,
...
typedef void (*ww_pfn_glEGLImageTargetTexture2DOES)(unsigned int target, void* image);
...
static int load_gl_dispatch(egl_gbm_state_t* st, ww_bridge_egl_get_proc_addr_fn get_proc) {
...
#pragma GCC diagnostic ignored "-Wpedantic"
    ...
    st->glFramebufferTexture2D =
        (ww_pfn_glFramebufferTexture2D)(void*)get_proc("glFramebufferTexture2D");
    st->glCheckFramebufferStatus =
        (ww_pfn_glCheckFramebufferStatus)(void*)get_proc("glCheckFramebufferStatus");
...
#pragma GCC diagnostic pop
    ...
    if (! st->glGenTextures || ! st->glDeleteTextures || ! st->glBindTexture ||
        ...
        ! st->glDeleteFramebuffers || ! st->glBindFramebuffer || ! st->glFramebufferTexture2D ||
        ! st->glCheckFramebufferStatus || ! st->glFlush || ! st->glFinish ||
        ! st->glEGLImageTargetTexture2DOES) {
        return -ENOSYS;
    }
    return 0;
...
static int gbm_probe_one_modifier(struct gbm_device* gbm, uint32_t fourcc, uint64_t modifier,
                                  uint32_t w, uint32_t h, uint32_t* out_planes) {
    uint64_t       mods[1] = { modifier };
    struct gbm_bo* bo =
        gbm_bo_create_with_modifiers2(gbm, w, h, fourcc, mods, 1, GBM_BO_USE_RENDERING);
    if (! bo) return -EIO;
...
static int probe_caps(ww_pool_t* pool, uint32_t width, uint32_t height) {
    ...
    for (size_t fi = 0; fi < sizeof(kCandidateFourccs) / sizeof(kCandidateFourccs[0]); ++fi) {
        ...
        struct gbm_bo* probe_bo = gbm_bo_create_with_modifiers2(
            st->gbm, width, height, fourcc, mod_buf, bag.n, GBM_BO_USE_RENDERING);
        ...
        for (size_t i = 0; i < bag.n; ++i) {
            uint32_t planes = 1;
            if (gbm_probe_one_modifier(st->gbm, fourcc, mod_buf[i], width, height, &planes) != 0) {
                continue;
            }
            if (n >= cap) break;
    ...
    pool->caps.color_caps   = WW_COLOR_ENC_SRGB | WW_COLOR_RANGE_LIMITED | WW_COLOR_ALPHA_PREMUL;
...
static EGLImageKHR import_with_modifier(egl_gbm_state_t* st, uint32_t fourcc, uint32_t w,
                                        uint32_t h, const int* fds, const uint32_t* strides,
                                        const uint32_t* offsets, uint32_t plane_count,
                                        uint64_t modifier) {
    ...
    attrs[k++]    = EGL_HEIGHT;
    attrs[k++]    = (EGLint)h;
    attrs[k++]    = EGL_LINUX_DRM_FOURCC_EXT;
    attrs[k++]    = (EGLint)fourcc;
    ...
    for (uint32_t p = 0; p < plane_count && p < 4; ++p) {
        attrs[k++] = kEglPlaneFd[p];
        attrs[k++] = fds[p];
        attrs[k++] = kEglPlaneOffset[p];
        attrs[k++] = (EGLint)offsets[p];
        attrs[k++] = kEglPlanePitch[p];
        attrs[k++] = (EGLint)strides[p];
        attrs[k++] = kEglPlaneModLo[p];
        attrs[k++] = mod_lo;
        attrs[k++] = kEglPlaneModHi[p];
        attrs[k++] = mod_hi;
    }
    attrs[k++] = EGL_NONE;
    return st->edt.eglCreateImageKHR(
        st->display, EGL_NO_CONTEXT, EGL_LINUX_DMA_BUF_EXT, NULL, attrs);
}

static EGLImageKHR import_without_modifier(egl_gbm_state_t* st, uint32_t fourcc, uint32_t w,
                                           uint32_t h, int dmabuf_fd, uint32_t stride,
                                           uint32_t offset) {
    EGLint attrs[] = {
        EGL_WIDTH,
        (EGLint)w,
...
static int alloc_slot(ww_pool_t* pool, uint32_t slot_index, ww_pool_slot_layout_t* out) {
    egl_gbm_state_t* st = (egl_gbm_state_t*)pool->backend_data;
    ...
    st->glFramebufferTexture2D(
        WW_GL_FRAMEBUFFER, WW_GL_COLOR_ATTACHMENT0, WW_GL_TEXTURE_2D, tex, 0);
...
static int populate_slot_view(ww_pool_t* pool, uint32_t slot_index, ww_pool_slot_t* out) {
    egl_gbm_state_t* st = (egl_gbm_state_t*)pool->backend_data;
    if (slot_index >= WW_POOL_MAX_SLOTS) return -EINVAL;
...
static int backend_init(ww_pool_t* pool, const void* init_data) {
    ...
    if (! init || ! init->egl_display || init->drm_render_fd < 0 || ! init->get_proc_address) {
        return -EINVAL;
    }
    ...
    if (! st->gbm) {
        ww_bridge_logf(WW_BRIDGE_LOG_ERROR, "ww_pool[egl_gbm]: gbm_create_device failed");
        free(st);
        return -EIO;
    }
